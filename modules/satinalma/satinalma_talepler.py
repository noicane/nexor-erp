...
class TalepSatirDialog(QDialog):
    ...
    def _setup_ui(self):
        ...
        self.lbl_stok.setStyleSheet(f"color: {brand.INFO}; font-size: 12px; font-weight: {brand.FW_BOLD}; padding: 4px;")
        form.addRow("Depo Stok:", self.lbl_stok)

        # Anlasma fiyati (read-only)
        self.lbl_anlasma = QLabel("-")
        self.lbl_anlasma.setStyleSheet(f"color: {brand.SUCCESS}; font-size: 12px; font-weight: {brand.FW_BOLD}; padding: 4px;")
        ...
        btn_kaydet.setStyleSheet(f"background: {brand.SUCCESS}; color: white; padding: {brand.SP_3}px {brand.SP_5}px; border-radius: {brand.R_SM}px; font-weight: {brand.FW_BOLD};")
    ...
    def _on_urun_changed(self, index):
        ...
        if stok and stok['miktar'] > 0:
            ...
            self.lbl_stok.setStyleSheet(f"color: {brand.SUCCESS}; font-size: 12px; font-weight: {brand.FW_BOLD};")
        else:
            self.lbl_stok.setText("Stok yok")
            self.lbl_stok.setStyleSheet(f"color: {brand.ERROR}; font-size: 12px; font-weight: {brand.FW_BOLD};")
...
class SatinalmaTaleplerPage(BasePage):
    ...
    def __init__(self, theme: dict):
        ...
        self.kullanici_id = YetkiManager._current_user_id or 1
        self.all_rows = []
        self.has_approval_authority = True  # Yetki kontrolu ileride eklenebilir
        self._setup_ui()
        self._load_data()
    ...
    def _setup_ui(self):
        ...
        btn_pdf = QPushButton("PDF Goster")
        btn_pdf.setCursor(Qt.PointingHandCursor)
        btn_pdf.setStyleSheet(f"background: {brand.INFO}; color: white; padding: {brand.SP_2}px {brand.SP_4}px; border-radius: {brand.R_SM}px;")
        btn_pdf.clicked.connect(lambda: self._yazdir())
        toolbar_layout.addWidget(btn_pdf)

        btn_siparise = QPushButton("Siparise Donustur")
        btn_siparise.setStyleSheet(f"background: {brand.SUCCESS}; color: white; padding: {brand.SP_2}px {brand.SP_4}px; border-radius: {brand.R_SM}px; font-weight: {brand.FW_BOLD};")
